...
local INVENTORY_SLOTS_PER_SLICE = 16
local METADATA_ITEMS_PER_SLICE = 8
...
local PLAN_OPERATIONS_PER_SLICE = 32
...
local function scanInventorySlice(job)
    ...
    for index = job.inventorySlotIndex, stop do
        ...
        if staticId ~= "None" then
            ...
            if job.exclusions[staticId] then
...
local function indexContainer(job, entry)
    ...
    if job.config.FillByChestFilter then
            ...
            if not isValid(entry.container) then
                ...
                for index = first, stop do
                    ...
                    if staticId == "None" then
                        ...
                        if metadata ~= nil and metadata.maxStack > 1 then
                            local stackCount
                            ...
                            if stackCount < metadata.maxStack then
...
local function newRouteState(job, item)
    local incubators = {}
    if item.isEgg and job.config.AltEggSorting and job.config.IncubatorsFirst then
    ...
    return {
        ...
        candidateIndex = 1,
        stages = {
            { kind = "incubator", entries = incubators },
            { kind = "normal", entries = job.containersByContainedItem[item.id] or {} },
            {
                kind = "normal",
                entries = job.config.FillByChestFilter
...
processNextRequest = function(job)
    ...
    local currentRouteOk = pcall(function()
        ...
        currentModule = currentConcrete:GetItemContainerModule()
    ...
    if not currentRouteOk or not isValid(currentModel) or not isValid(currentConcrete)
        or not isValid(currentModule) or not isValid(currentContainer)
...
function QuickStack.configure(config, logger, debugLogger)
    ...
    Notifications.configure(logger)
end
...
function QuickStack.begin()
    ...
    local blocked, inputError = P.blockingUiOwnsInput(controller)
