...
实机诊断确认本地运行时可读取
`UPalShopManager.CreatedItemShopMap_ForServer`，当次共有 9 个已登记 item shop。
结合下文已证实的服务端控制流——出售只校验 `ShopID` 是否仍在该 map 中，命中后不再
解引用 shop，也不检查 UI、距离或 vendor actor——F5 现在优先从该 map 选择一个真实
条目，并在提交前重新枚举同一条目。当前基地商人发现只作为客户端无法读取登记表或
登记表没有可读条目时的兼容回退。此修正取代下文早期“必须先找到当前基地商人”的
实现建议，但不改变禁止伪造 ShopID、跨 world 缓存或直接删除物品的安全边界。

## 结论

当前版本将自动出售作为 F5 Quick Stack 存储路由前的独立阶段，优先使用服务端
已经登记的真实 item shop；当前基地商人发现仅作为兼容回退。不应构造伪 ShopID、
跨 world 缓存商店或直接删除物品。出售后先等待并重读源槽；一旦出售 RPC 已提交，
任何未确认的剩余物品都留在背包而不再进入本次存储路由，避免迟到的出售与同槽位
移动请求并发。提交前未找到可用商人时，默认将待售物品保留在背包；用户可以关闭
该保护，让它们继续按普通归箱规则处理。
...
可以实现的产品行为是：用户明确开启开关后，F5 在存储路由前，从服务端已登记
item shop 中选择并重新验证一个真实 ShopID，再提交白名单出售 batch；确认源槽结果
后再继续归箱，未确认的待售物品留在背包并明确报告。没有合法商店上下文时跳过出售，
并根据用户设置将待售物品保留在背包或交回普通归箱。静态与运行时证据均不支持伪造
ShopID 或直接扣除物品，该路径继续明确禁止。
