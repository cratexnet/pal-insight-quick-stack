...
$object = Join-Path $build 'pal_insight_quick_stack_settings_input.obj'
& $cl /nologo /c /O2 /std:c++20 /EHsc /W4 /WX /MT `
    /D_CRT_SECURE_NO_WARNINGS @includes `
    (Join-Path $nativeRoot 'pal_insight_quick_stack_settings_input.cpp') "/Fo:$object"
if ($LASTEXITCODE -ne 0) { throw 'Quick Stack settings input helper compilation failed.' }

$output = Join-Path $bin 'PalInsightQuickStackSettingsInput.dll'
& $link /nologo /DLL /OPT:REF /OPT:ICF /INCREMENTAL:NO /Brepro `
    "/DEF:$($nativeRoot)\PalInsightQuickStackSettingsInput.def" "/OUT:$output" `
    @libraries $object
if ($LASTEXITCODE -ne 0) { throw 'Quick Stack settings input helper link failed.' }
